...
def read (self, ddz=False, average=False, span=10, compute_raw=True, box_average=False,upper_limit = 1e6, 
		...
		plot_levels=45, stdev=False, minimum=False,nocache=False,parallel=False):
	...
	if span:
		...
		if not make_file:
			X.append(self.data.end) # there is no guarantee that this last one will receive any data
			Z.append([])
	...
	count = 0 # just to count the total number of shots received for the request period.
	now_bin = False
	###  HERE IS WHERE WE CHECK THE CACHE, AND CHECK THE CACHE VERSION TO MAKE SURE IT IS UP TO SPUNK
	if not clouds and not nocache and os.path.exists(self.proj.cache+"/cl31_msg2_"+self.nowPkg.lower().replace(' ','')+".uud"):
	 #FIXME - this system could not handle clouds...
	 # then check if the cache file for this stream exists, unless we are told not to cache, read it
	 # test to see if the file exists
	 cache = open(self.proj.cache+"/cl31_msg2_"+self.nowPkg.lower().replace(' ','')+".uud")
 	 if cache:
		print "Hooray!! I can read from cache!"
		# well, now we can just read from the cache file, so woohoo!
		current_bin_key = 0
		#Z = [[]] # give the first bin its contents
		y_ed = False
		#Z = [[] for x in X]
		for line in cache:
			if len(line) < 500: continue
			# each line is an ob
			t = int(line[0:10]) # lets see if pre-splitting is faster - it may be c-coded
			if t < self.data.begin - ceil_buffer or t > self.data.end + ceil_buffer:
				continue
			# now read the line
			# first determine which bin i need to be in
			# bin times are determined by +/- span/ 2 seconds from x[current_bin_key]
			p = line.split("|")
			key = -1
			d = span*60/2.

			if math.fabs(t-X[current_bin_key]) > d:
				# well, then i guess we need to revise the value
				# first look forward
				key = current_bin_key
				gt = False # got it catch
				for tm in X[current_bin_key+1:]:
					# well, now we are looping forward
					key += 1
					if t >= tm + d or t < tm - d: continue
					# then this is our bin
					current_bin_key = key
					gt = True
					break
				# well, now we can look back if we need to
				if not gt:
					key = -1
					for tm in X:
						# loop up, the normal way
						key += 1
						if t >= tm + d or t < tm - d: continue
						# then this is our bin
						current_bin_key = key
				...
				if not gt:
					# well, crap
					print "Shoot, I seem to have a binning problem...",t,X[current_bin_key] - t,d
				self.log("Building bin ",current_bin_key,'of',len(X))
			#"""
			#print "huh?",current_bin_key
			lp = p[2].strip().split(' ')
			Z[current_bin_key].append(array.array('f')) # append the list for this shot to the bin
			key = -1
			count += 1 # increment the counter for how many profiles we have received
			for val in lp:
				key += 1
				if key*GATE_HEIGHT > max_height:
					break
				val = int(val,16) # bust it out of hexidecimal!!
				...
				Z[current_bin_key][-1].append(val)
	 ...
	 for file_and_dir in self.find_files('ncar_iss_vaisala_ceilometer','.DAT',allow_repeats=False):
		...
		if '.swp' in fname or fname[0] == '.': continue # avoid opening swp files...
		# THERE MAY BE VARIOUS NAMING CONVENTIONS, SO I WILL TRY TO ACCOUNT FOR THOSE I HAVE ENCOUNTERED
		# so far they fit the profile...
		ftime = calendar.timegm(time.strptime("201"+fname[1:-4]+"UTC","%Y%m%d%H%Z"))
		start = False
		# files last for 6 - 24 hours, so if the end of the file is within the time, then it is still good
		if ftime + 25*3600+ceil_buffer < self.data.begin or ftime > self.data.end+ceil_buffer:
			continue
		...
		for line in open(file_and_dir).readlines():
				...
				if "-20" in line:
					...
					# check if we are at the end...
						...
						if span:
							...
							for i in range(current_bin - 1,lenr(X)): # it is somewhere between the current length of Z and the length of X
			...
			count += 1 # add one to the counter to know just how many shots were received for this request
			while i < len(line):
				val = line[i:i+5]
				key = i/5
				i+= 5
	...
	### SO IF THERE IS NO DATA CURRENTLY, THEN THIS DATA IS NOT IN CL31_MSG2 FORMAT, THROW TO CT25K_MSG2 FORMAT 
	if len(Y) == 0:
		print "Uhoh, I was not able to read any of the data in this file, I suspect that it was not a CL31 msg 2 formatted data."
		print "I am going to pass you off to the CT25K msg 2 ingestor to see if it can read this file"
		exec "import uudewey.formats.vaisala.ct25k_msg2 as c"
		# do not bounce right now
		"""
		return c.run(self, ddz=ddz, average=average, span=span, compute_raw=compute_raw, box_average=box_average,upper_limit = upper_limit, 
		lower_limit=lower_limit, max_height=max_height, ceil_buffer=ceil_buffer, scaled=scaled, 
		make_file=make_file, make_file_name=make_file_name, make_file_raw=make_file_raw,still_plot=still_plot,
		ddz_extra=ddz_extra,clouds=clouds,no_bs=no_bs,exp_filter=exp_filter,nans2val=nans2val,nm=nm,nm_pct=nm_pct,
		plot_levels=plot_levels, stdev=stdev, minimum=minimum)
		"""
	...
	if ddz_extra:
		data.new('data',fill=flip2d(out))
		data.new('ddz',fill=flip2d(out2))
	else:
		data.new('data',fill=flip2d(out))
	# clouds are returned to the data object, so no need to return further...
	print count,'shots processed into',len(X),'bins'
	return data
...
def compile(obj,pkg):
	location = obj.proj.cache
	# save the file you create to this directory
	out = open(location+"/cl31_msg2_"+pkg+".uud",'w')
	out.write('1.0.0|10\n') # data string object, since the file is also a string
	#FIXME - this format is more playful, and does not record status or cloud info!! LOSES DATA!!!AHH!
	# there is currently no set format, so just save it the way it wants to be saved
	# the first line should be some juicy info though... like version and whatnot
	for file_and_dir in obj.find_files('ncar_iss_vaisala_ceilometer','.DAT',allow_repeats=False):
		f = open(file_and_dir,'r')
		print 'opening',f
		go = False
		for line in f:
			data = ''
			...
			if len(line) < 500:
				if "-20" in line:
					...
					continue
				if go:
					go += line + "^" # accumulate the data pre-profile
				continue
			# then that makes this a data line, so save obtime, and the line
			# break the line down into its hex segments, but keep it in hex, because that is more efficient
			data += str(obtime)+"|"+go+"|"
			for i in xrange(0,len(line), 5):
				data += " "+line[i:i+5]
			data += "\n"
			go = False
			out.write(data) # lets see how writing every time works, if that is easier or harder for the system.
		f.close()
