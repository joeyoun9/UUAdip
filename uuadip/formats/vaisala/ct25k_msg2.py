"""
# UTDewey package for processing raw data produced by the a Vaisala CT25K ceilometer (or it's data format)
# current configuration is for the CL25K Ceilometer, and it's raw data format
...
def run (self, ddz=False, average=False, span=10, compute_raw=True, box_average=False,upper_limit = 1e6, 
	...
	for file_and_dir in self.find_files('ncar_iss_vaisala_ceilometer','.DAT',allow_repeats=False):
		...
		for line in open(file_and_dir).readlines():
			...
			if len(info_lines) < 3: # the data starts on line 4!
				if "-20" in line:
					info_lines = []
