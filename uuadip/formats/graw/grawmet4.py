...
from uudewey.tools.metcalcs.sonde import sounding_dewpoint
...
def run (self, site=False, proc=False, wind_averaging=20, cutoff=True):
	...
	gs_out = {'time':[],'keys':[],'tc':[],'tdc':[],'rh':[],'tv':[],'rs':[],'gp':[],'p':[],'wdir':[],'wspd':[],'lat':[],'lon':[],'z':[],'init':[],'time_dim':[]}
	# the internal holder for the data
	if site:
		site = site.lower().replace(' ','')
	#self.data.set("site",site)
	do = self.DO(['z','p','gp','time'],'...',self,)

	#this object has 3 vertical data dimensions, and this is a square set so each vertical dimension should have the same shape as the data
	# time is still one dimensional
	...
	if proc:
		...
		proc_files = self.find_files('other req','txt',aux=True) # i didnt want to do the .txt restriction, but no binaries
		processed_files = {}
		for f in proc_files:
	...
	for file_loc in self.find_files('graw_rawinsondes','.GD',allow_repeats=False):
		...
		#gs_out.append({}) # append a new sounding dictionary!
		...
		if proc:
			...
			gs_out['time'][num] = tm_l
			gs_out['keys'][num] = range(len(tm_l))
			gs_out['tc'][num] = t_l
			gs_out['tdc'][num] = td_l
			gs_out['rh'][num] = rh_l
			gs_out['tv'][num] = tv_l
			...
			gs_out['gp'][num] = gp_l
			gs_out['p'][num] = p_l
			gs_out['wdir'][num] = dir_l
			gs_out['wspd'][num] = spd_l
			gs_out['lat'][num] = lt_l
			gs_out['lon'][num] = ln_l
			gs_out['z'][num] = z_l
			...
			continue
		# END IF PROC - end if this is just reading processed files
		...
		gs_out['init'][num] = init
		gs_out['time_dim'].append(init[0])
		...
		gs_out['rh'][num] = rh_l
		gs_out['tv'][num] = tv_l
		gs_out['rs'][num] = [False for x in tm_l]
		gs_out['gp'][num] = [False for x in tm_l]
		gs_out['p'][num] = p_l
		gs_out['wdir'][num] = dir_l
		gs_out['wspd'][num] = spd_l
		gs_out['lat'][num] = lt_l
	...
	do.setdim('time',gs_out['time_dim'])
	do.setdim('z',gs_out['z'])
	do.setdim('p',gs_out['p'])
	do.setdim('gp',gs_out['gp'])
	do.new('time',fill=gs_out['time'])
	do.new('init',fill=gs_out['init'])
	do.new('keys',fill=gs_out['keys'])
	do.new('temperature',fill=gs_out['tc'])
	do.new('dewpoint',fill=gs_out['tdc'])
	do.new('relh',fill=gs_out['rh'])
	do.new('tv',fill=gs_out['tv'])
	do.new('rs',fill=gs_out['rs'])
	do.new('gp',fill=gs_out['gp'])
	do.new('pressure',fill=gs_out['p'])
	do.new('wdir',fill=gs_out['wdir'])
	do.new('wspd',fill=gs_out['wspd'])
	do.new('lat',fill=gs_out['lat'])
	do.new('lon',fill=gs_out['lon'])
	do.new('z',fill=gs_out['z'])
