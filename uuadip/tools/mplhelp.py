"""
	the matplotlib artist assistant for plotting formatting, 
	more specifically, this is to help me remember/record the nifty things I was able to do with timestamps and whatnot

	many of htese functions will involve the other axes, and will more act to wrap matplotlib functions into a more synthesized version of the same thing
	a bit redundant, but it will make the whole process of rewriting the plots much simpler I believe.

	This will generally only use functions/def's because classes would be a bit tedious for this operation...
"""

# we will import a vast majority of the modules that could be used. Again, this could be seen as ineffieient, but it will be easier.
...
def ttUTC(ax,xy,begin,end,ms=major_scale,mns=minor_scale,sms=True):
	timeticks(ax,xy,tz.utcTZ(),begin,end,days=True,major_scale=ms,minor_scale=mns,sms=sms)

def ttMST(ax,xy,begin,end,ms=major_scale,mns=minor_scale,sms=True):
	timeticks(ax,xy,tz.mstTZ(),begin,end,major_scale=ms,minor_scale=mns,sms=sms)


def timeticks(ax,xy,tzone,begin,end,days=False,major_scale=major_scale,minor_scale=minor_scale,sms=True):
	"""
